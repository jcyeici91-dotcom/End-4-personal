...
MouseArea {
    ...
    // ========================================================================
    //  ✅ PANEL DE CONTROL (AJUSTES RÁPIDOS)
    //  Aquí puedes ACTIVAR/DESACTIVAR efectos y ajustar espacios.
    //
    //  --- Para "acercar cada dato" (que queden más juntos) toca estos 4:
    //   1) resourcesSpacingH / resourcesSpacingV      -> espacio ENTRE cada bloque (RAM / CPU / GPU / CPU%)
    //   2) resourceInnerSpacingH / resourceInnerSpacingV -> espacio DENTRO de cada bloque (icono/texto vs puntito)
    //   3) outerPaddingH / outerPaddingV              -> padding general del widget (margen externo)
    //   4) resourceLeftMarginH / resourceLeftMarginV  -> margen extra antes de ciertos items (CPU temp/GPU temp/CPU uso)
    //
    //  Nota: puedes usar valores 0 o incluso negativos (con cuidado) en Layout.*Margin
    // ========================================================================

    // --- ORIENTACIÓN / LAYOUT ---
    property bool vertical: false

    // Vertical: rotar el layout horizontal en modo vertical (como lo tienes)
    property bool rotateInVertical: true
    property int verticalRotationDegrees: -90

    // --- ESPACIADO (esto es lo que te acerca/aleja todo) ---
    // Espacio ENTRE recursos (RAM | CPU Temp | GPU Temp | CPU Uso)
    ...
    // Espacio DENTRO de cada recurso (Resource + puntito)
    ...
    // Padding externo total del widget
    ...
    // Margen extra a la izquierda (se usa para separar un poco algunos recursos)
    ...
    // --- EFECTOS / TOGGLES ---
    property bool enableEffects: true          // Master switch (puntitos animados, etc.)
    property bool enableTooltips: true         // Tooltips (se desactivan en vertical automáticamente)

    // Gato
    property bool showCat: true                // Mostrar el bloque del gato (espacio/cargador)
    property bool enableCatGif: true           // Cargar el gif (si false y reserveCatSpace true, reserva el hueco)
    property bool enableCatEffects: true       // Permite que corra según CPU
    property bool reserveCatSpace: false       // Mantener hueco aun si no cargas el gif
    ...
    // --- PUNTITO (DOT) AJUSTES ---
    // Si quieres el puntito más grande/pegado, toca aquí (sin romper nada más).
    property int dotSize: 6
    property int dotExtraBox: 10             // padding interno del componente (size + 12). Baja si lo quieres más compacto.
    ...
    // ========================================================================
    //  ESTADO DERIVADO (no tocar normalmente)
    // ========================================================================
    ...
    // ------------------------------------------------------------------------
    // Abrir btop con click izquierdo
    // ------------------------------------------------------------------------
    Process {
        id: btopProc
    ...
    component PulsingDot: Item {
        ...
        // Tamaño del "contenedor" del dot (esto afecta lo compacto que queda)
    ...
    component ResourceWithDot: RowLayout {
        ...
        // Tooltip por item (solo horizontal)
        MouseArea {
            anchors.fill: parent
        ...
        PulsingDot {
            Layout.alignment: Qt.AlignVCenter
            // Si quieres que el puntito quede MÁS pegado al texto/icono, ajusta estos márgenes:
    ...
    Component {
        ...
        Item {
            ...
            // --- Rotated variant (lo que estabas usando) ---
            Item {
                id: rotContainer
            ...
            // --- Non-rotated variant (lo dejé intacto, por si luego lo usas) ---
            ColumnLayout {
                id: columnBlock
                visible: !root.rotateInVertical
                anchors.centerIn: parent
