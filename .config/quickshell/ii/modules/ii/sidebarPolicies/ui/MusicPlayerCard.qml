...
Item {
    ...
    // --- i18n strings (dependen de Settings -> Config.options.language.ui) ---
    readonly property string trPlayerFallback: Translation.tr("Music.Player")
    readonly property string trNoPlayback: Translation.tr("Music.NoPlayback")
    readonly property string trSearchingLyrics: Translation.tr("Lyrics.Searching")
    readonly property string trNoLyrics: Translation.tr("Lyrics.NoLyrics")
    ...
    // --- COLORES ---
    property color artDominantColor: ColorUtils.mix((colorQuantizer?.colors[0] ?? Appearance.colors.colPrimary),
    ...
    function _looksLikeNoLyrics(txt) {
        if (!txt) return true
        var t = String(txt)
        ...
        if (low.includes("no hay letras")) return true
        if (low.includes("buscando")) return true
        if (low.includes("no lyrics")) return true
        if (low.includes("searching")) return true

        // Y también filtramos las traducciones que mostramos en UI:
        if (t.indexOf(root.trNoLyrics) !== -1) return true
        if (t.indexOf(root.trSearchingLyrics) !== -1) return true

        // Texto demasiado corto -> probablemente no es una línea real
    ...
    property bool hasSyncedLyrics: {
        ...
        return !_looksLikeNoLyrics(lyricsEngine.displayText)
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            Text {
                text: root.player?.identity || root.trPlayerFallback
                font.pixelSize: 14
                font.bold: true
                color: "#eeeeee"
                opacity: 0.9
                ...
                RowLayout {
                    ...
                    ColumnLayout {
                        ...
                        MarqueeText {
                            ...
                            text: StringUtils.cleanMusicTitle(root.player?.trackTitle) || root.trNoPlayback
                            font.pixelSize: 18
                            font.bold: true
                            color: "#ffffff"
                        }
                ...
                Item {
                    ...
                    Item {
                        ...
                        Text {
                            ...
                            // Texto que se muestra en modo karaoke (traducible)
                            text: root.hasSyncedLyrics ? lyricsEngine.displayText : root.trSearchingLyrics
        ...
        RowLayout {
            ...
            ControlButton {
                ...
                color: {
                    if (root.lyricsMode) return Qt.rgba(1, 1, 1, 0.2)
                    if (root.hasSyncedLyrics && root.player.playbackState === MprisPlaybackState.Playing) return pulseColor
                    return "transparent"
                }
                ...
                SequentialAnimation on pulseColor {
                    ...
                    ColorAnimation {
                        ...
                        to: Qt.rgba(root.artDominantColor.r, root.artDominantColor.g, root.artDominantColor.b, 0.4)
                        duration: 1000
                        easing.type: Easing.InOutQuad
                    }
                    ColorAnimation {
                        from: Qt.rgba(root.artDominantColor.r, root.artDominantColor.g, root.artDominantColor.b, 0.4)
